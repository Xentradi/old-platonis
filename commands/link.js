exports.run = async (client, message, args) => {
	...
	const data = client.warframe.warframes[name] || client.warframe.mods[name] || client.warframe.abilities[name];
	if (!data) {
		client.udf.commandReact(message, 0);
		return message.reply('That\'s not a linkable item, did you spell it correctly?');
	}
	if (data.Health) {
